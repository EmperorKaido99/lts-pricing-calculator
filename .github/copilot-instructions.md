# LTS Pricing Calculator - GitHub Copilot Instructions
...
When the user says **"StartTask LTSPC-XXXX"** (or similar), initiate the D5 workflow:

> 💡 **Model Hint:** Use a **thinking/reasoning model** (e.g., Claude Opus, o3) for the **Define** and **Discover** phases — these require deep reasoning and thorough investigation. Once the implementation plan is **approved**, switch to a **faster implementation model** (e.g., Claude Sonnet, GPT-4o) for the **Deliver** grunt work.
...
3. Create a task tracking file at `memorybank/activeTasks/LTSPC-XXXX.md` using the template below.
...
2. For each incomplete task, read its task file at `memorybank/activeTasks/LTSPC-XXXX.md`.
...
- Capture the ticket details (LTSPC-XXXX).
...
- **Update `LTSPC-XXXX.md` incrementally** after each discovery step — do NOT wait until the end.
...
When creating `memorybank/activeTasks/LTSPC-XXXX.md`, use this structure:
...
## Discover
*(Updated progressively — each section filled in as discovery proceeds)*
